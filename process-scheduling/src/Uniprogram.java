...
public class Uniprogram {
	...
	//array that holds element that is getting processed
	ArrayList<Process> oneProcess;
	...
	Uniprogram(ArrayList<Process> arrayProcessInput, boolean verboseFlag){
		...
		//update state of every process 
		//pick a process and run till completion
		oneProcess = new ArrayList<Process>();
		for (int i = 0; i < arrayProcess.size(); i++) {
			//<<<Three things to update to prepare to run a process>>>
			//step 1: get the next process from sorted array
			Process process = arrayProcess.get(i);
			
			//step 2: change status so this process can be scheduled
			process.toSchedule = true;
			
			//step 3: add this process to array to execute
			oneProcess.add(process);
			//System.out.println("process" + process.numberListedInput + ": " + process.processToString() + " and toSchedule is: " + process.toSchedule + " and state is: " + process.state);
			
			//System.out.println("going to MoveToQ....");
			
			//move toSchedule process into ready Q
			moveToReadyQ();
			
			//System.out.println("going to run to complete....");
			//<<<<<<< BEGIN RUNNING >>>>>>>
			runToCompletion();
			
			//System.out.println("complete running...");
			
			//<<<<<< END RUN FOR CURRENT PROCESS >>>>>>>
			
			//Three things to update after process is done executing
			//step 1: change status to false, so it will not get selected when printing or moveToQ
			process.toSchedule = false;
			
			//step 2: reset terminateQ to size 0
			this.terminateQ.clear();
			
			//step 3: reset oneProcessQ to size 0 
			oneProcess.clear();
	...
	public void runToCompletion(){
		...
		//uniprogramming - once a process terminates end running
		while(this.terminateQ.size() < 1) {
			
			//<<<<<<<< CPU IS IDLE >>>>>>>>
			//if readyQ is empty, wait for to get unblock
			while(this.readyQ.size() == 0 && (this.terminateQ.size() < 1)) {
			...
			//readyQ size is 0 when process blocked or terminated
			if (readyQ.size() > 0) {
		...
		}//end_while - run to completion finish
	...
	public void cpuExecute(Process process) {
		...
		for (int i = 0; i < initialCpuBurst; i++, process.reduceCpuBurst()) {
			...
			//check if process finishes its burst. If it does, block. 
				...
				//System.out.println("block time: " + process.ioBurst);
	...
	public void printProcess(){
		...
		for (int i = 0; i < arrayProcess.size(); i ++) {
			Process process = arrayProcess.get(i);
			//process arrived, but not scheduled yet
			if (process.state == "created" && totalTime > process.arrivalTime) {
			...
			//WHAT??
			else if (process.toSchedule == true && process.state == "ready") {
				System.out.format("%13s%3d", "ready", 0);
			}
	...
	public void moveToReadyQ(){
		...
		//need to check if the one process gets unblocked
		...
		for (int i = 0; i < this.arrayProcess.size(); i++) {
			...
			//look at the process that is currently scheduled
			if(process.toSchedule == true) {
		...
		//use lab 2 tie break NOT NEEDED
		//ArrayList<Process> processed = tieBreak(toProcess);
		
		ArrayList<Process> processed = toProcess;
