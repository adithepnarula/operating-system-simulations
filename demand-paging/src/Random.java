...
public class Random {
	...
	public Random(ArrayList<Process> processArray, ArrayList<PageFrame> framesArray){
		this.time = 1;
		...
		while(!allTerminated(processArray)){
			...
			for(int outer = 0; outer < processArray.size(); outer++) {
				...
				for(int i = 0; i < 3 && (curProcess.terminated == false); i++){
					...
					System.out.print(curProcess.id + " references word " + curProcess.nextRef + " (virtual page "+ virtualPageNum + ") at time " + this.time + ": ");
					...
					if(found == false){
						...
						//Random - randomly remove  
						if(highest == -1){
							
							
							//get the next random number
							int randomNumber = this.randomFile.getNum();
							int rIndex = randomNumber %framesArray.size();
							//System.out.println("pick extra random: " + randomNumber);
							
							
							//get the first element in the frame
							int randomProcessID = framesArray.get(rIndex).ownedBy;
							int randomProcessVP = framesArray.get(rIndex).virtualPageNum;
							
							
							//get the process that owns the first page
							Process randomProcess = processArray.get(randomProcessID-1);
							
							//get process load time
							int loadTime = randomProcess.vpArray.get(randomProcessVP).loadTime;
							
							//find how long it was in physical memory
							int timeInPm = this.time - loadTime;
							
							System.out.println("Fault, evicting page " + randomProcessVP + " of process " + randomProcessID + " from frame " + rIndex);
							
							//add to resident time
							randomProcess.residentTime = randomProcess.residentTime + timeInPm;
							
							//add evict
							randomProcess.numEvict++;
							
							//add page
							framesArray.get(rIndex).ownedBy = curProcess.id;
							framesArray.get(rIndex).virtualPageNum = virtualPageNum;
